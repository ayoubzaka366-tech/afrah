...
export default function ManageProductCategories() {
  ...
  const fetch = () => {
    API.get('/product-categories').then(res => setCategories(res.data)).catch(() => {}).finally(() => setLoading(false));
  };
      ...
      <FormModal open={showModal} onClose={() => setShowModal(false)}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
            ...
            placeholder="Titre (ex: Dessiner sur du verre)" required />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map(cat => (
            <div key={cat.id} className="bg-white rounded-2xl shadow-sm border border-gray-50 overflow-hidden group">
              <div className="h-36 bg-gradient-to-br from-emerald-900 via-teal-800 to-cyan-900 flex items-center justify-center">
                {cat.image ? (
                  <img src={`/uploads/products/${cat.image}`} alt={cat.title} className="w-full h-full object-cover" />
                ) : (
